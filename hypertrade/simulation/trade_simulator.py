...
from hypertrade.simulation.market_time import (
    add_market_minutes,
    add_market_minutes_cached,
    compute_entry_time,
    compute_entry_time_cached,
)
...
def _psar_init(direction, entry_high, entry_low, psar_step):
    if direction == LONG:
        return entry_low, entry_high, psar_step, True
    return entry_high, entry_low, psar_step, False


@nb.njit
def _psar_update_and_check(direction, o, h, l, psar, ep, af, bull, prev_low1, prev_low2, prev_high1, prev_high2, psar_step, psar_max):
    ...
    if bull:
        ...
        if psar > prev_low2:
            psar = prev_low2
        if l <= psar:
            return True, ep, l, psar_step, False, min(ep, o), 3
        if h > ep:
            ep = h
        ...
        if psar < prev_high2:
            psar = prev_high2
        if h >= psar:
            return True, ep, h, psar_step, True, max(ep, o), 3
        if l < ep:
            ep = l
            af = min(af + psar_step, psar_max)
...
@nb.njit
def simulate_trade_core(dt_ns, ohlc, entry_idx, exit_deadline_ts, direction, sl_pct, tp_pct, psar_enabled, psar_step, psar_max, ts_enabled, ts_dist, slippage, commission):
    ...
    psar, ep, af, bull = _psar_init(direction, entry_high, entry_low, psar_step) if psar_enabled else (0.0, 0.0, psar_step, True)
    trail, best = _ts_init(direction, entry_price, ts_dist) if ts_enabled else (0.0, 0.0)
    had_exit = False
    for i in range(entry_idx + 1, ohlc.shape[0]):
        ts, o, h, l, c = dt_ns[i], ohlc[i, 0], ohlc[i, 1], ohlc[i, 2], ohlc[i, 3]
        hit, px, rsn = _check_time_exit(ts, exit_deadline_ts, o)
        if hit:
            exit_price, exit_idx, reason, had_exit = px, i, rsn, True
            break
        if psar_enabled:
            hit, psar, ep, af, bull, px, rsn = _psar_update_and_check(direction, o, h, l, psar, ep, af, bull, prev_low1, prev_low2, prev_high1, prev_high2, psar_step, psar_max)
            if hit:
                exit_price, exit_idx, reason, had_exit = px, i, rsn, True
                break
        if ts_enabled:
            ...
            if hit:
                exit_price, exit_idx, reason, had_exit = px, i, rsn, True
                break
        hit, px, rsn = _check_sl_tp(direction, c >= o, o, h, l, sl_price, tp_price)
        if hit:
            exit_price, exit_idx, reason, had_exit = px, i, rsn, True
            break
        prev_low2, prev_low1 = prev_low1, l
        prev_high2, prev_high1 = prev_high1, h
    if not had_exit:
...
def simulate_trade(symbol, signal_time, params, ohlc, direction=LONG, market_cache=None):
    try:
        from hypertrade.signals import filters

        entry_dt = compute_entry_time_cached(signal_time, params.delay_open, market_cache) if market_cache is not None else compute_entry_time(signal_time, params.delay_open)
        ...
        if market_cache is not None:
            ...
            exit_deadline_ts = np.int64(add_market_minutes_cached(entry_ns, int(params.holding_minutes), market_cache))
        else:
            exit_deadline_ts = np.int64(pd.Timestamp(add_market_minutes(pd.Timestamp(entry_dt), int(params.holding_minutes))).value)

        ohlc_np = np.column_stack([ohlc["open"].values, ohlc["high"].values, ohlc["low"].values, ohlc["close"].values]).astype(np.float64)
        ...
        if bool(getattr(params, "atr_use", False)):
            ...
            indicator_config = getattr(params, "indicator_config", {})
        ...
        hold_minutes = float(hold_bars * int(getattr(params, "bar_minutes", 15)))
        return {
            "symbol": symbol,
        ...
        return {"symbol": symbol, "rejected": True, "reject_reason": str(exc)}
