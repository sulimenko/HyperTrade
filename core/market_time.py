...
def compute_entry_time(signal_dt: datetime, delay_minutes: int) -> datetime:
    ...
    market_open_et = eastern.localize(datetime.combine(signal_dt.date(), time(9, 30)))
    market_close_et = eastern.localize(datetime.combine(signal_et.date(), time(16, 0)))
    ...
    if signal_et < market_open_et:
        ...
        next_day = signal_et.date() + timedelta(days=1)
