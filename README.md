...
Create a standard local Python environment:
...
`.conda/` and `.venv/` are local ignored directories. If a stale local `.conda/` directory exists, remove it manually outside git with `rm -rf .conda`.
